...
(define hasher
  (lambda (hashlist dictionary)
    (if (null? hashlist)
        '()
        (append (map (car hashlist) dictionary) (hasher (cdr hashlist) dictionary)))
))
...
(define gen-checker
  (lambda (hashfunctionlist dictionary)
    (let ((bitvector (hasher hashfunctionlist dictionary)))
      (lambda (word)
        (compare bitvector (hasher hashfunctionlist (list word)))))
))
